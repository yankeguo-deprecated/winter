...
func completelyUnsafeNotConstantTimeDecode(buf []byte) ([]byte, error) {
	if len(buf) == 0 {
...
func TestDecodeBlackBox(t *testing.T) {
	t.Parallel()
	err := quick.CheckEqual(Decode, completelyUnsafeNotConstantTimeDecode, nil)
	if err != nil {
		t.Error(err)
	}
}
